 ...
 */
package hcl.jp.trivaldrive.billingrepo
...
import hcl.jp.trivaldrive.billingrepo.BillingRepository.GameSku.CONSUMABLE_SKUS
import hcl.jp.trivaldrive.billingrepo.BillingRepository.GameSku.INAPP_SKUS
import hcl.jp.trivaldrive.billingrepo.BillingRepository.GameSku.SUBS_SKUS
import hcl.jp.trivaldrive.billingrepo.localdb.AugmentedSkuDetails
import hcl.jp.trivaldrive.billingrepo.localdb.Entitlement
import hcl.jp.trivaldrive.billingrepo.localdb.GAS_PURCHASE
import hcl.jp.trivaldrive.billingrepo.localdb.GasTank
import hcl.jp.trivaldrive.billingrepo.localdb.GoldStatus
import hcl.jp.trivaldrive.billingrepo.localdb.LocalBillingDb
import hcl.jp.trivaldrive.billingrepo.localdb.PremiumCar
...
class BillingRepository private constructor(private val application: Application) :
    ...
    fun queryPurchasesAsync() {
        ...
        if (isSubscriptionSupported()) {
            ...
            playStoreBillingClient.queryPurchaseHistoryAsync(BillingClient.SkuType.SUBS, PurchaseHistoryResponseListener { billingResult, purchaseHistoryRecordList ->
                {
                   if(billingResult.responseCode.equals(BillingClient.BillingResponseCode.OK) && !purchaseHistoryRecordList.isEmpty()) {
                       purchaseHistoryRecordList.forEach { purchaseHistoryRecord: PurchaseHistoryRecord? ->
                           {
                               Log.d(LOG_TAG, "Product [SKU]: ${purchaseHistoryRecord?.sku}")
                           }
                       }
    ...
    private fun querySkuDetailsAsync(
        ...
        //val params = SkuDetailsParams.newBuilder().setType(skuType).build()
    ...
    fun launchBillingFlow(activity: Activity, skuDetails: SkuDetails) {
        ...
        val purchaseParams = BillingFlowParams.newBuilder().setSkuDetails(skuDetails)
                .setOldSku(oldSku)
                .setReplaceSkusProrationMode(BillingFlowParams.ProrationMode.UNKNOWN_SUBSCRIPTION_UPGRADE_DOWNGRADE_POLICY).build()
    ...
    private object GameSku {
       ...
       // val INAPP_SKUS = listOf(GAS, GAS1, PREMIUM_CAR)
        val INAPP_SKUS = listOf(GAS, GAS1)
        val SUBS_SKUS = listOf(GOLD_MONTHLY, GOLD_YEARLY, PROMO_1, PROMO_2)
